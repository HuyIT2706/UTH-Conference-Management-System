...
import { useGetSubmissionByIdQuery, useUpdateSubmissionStatusMutation } from '../../redux/api/submissionsApi';
...
const SubmissionDetailSection = memo(({ submissionId }: SubmissionDetailSectionProps) => {
  ...
  const canMakeDecision = isChairOrAdmin && hasReviews && 
    (submission?.status === 'REVIEWING' || (submission?.status === 'SUBMITTED' && hasReviews));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 rounded-lg p-6">
        ...
        <div className="space-y-3">
          ...
          {submission.status === 'CAMERA_READY' && submission.cameraReadyFileUrl ? (
            <div>
              <p className="text-sm font-medium text-gray-700 mb-2">File bản hoàn thiện:</p>
              <button
                onClick={() => window.open(submission.cameraReadyFileUrl, '_blank')}
      ...
      {canMakeDecision && (
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Quyết định</h3>
