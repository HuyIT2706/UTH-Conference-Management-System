...
const CameraReadyPage = () => {
  ...
  // Fetch submissions - only get CAMERA_READY status
  const {
  ...
  // Filter by status (only CAMERA_READY) and search query
  const searchFilteredSubmissions = useMemo(() => {
    // First filter by status - only show CAMERA_READY
    const cameraReadySubmissions = allSubmissions.filter(
      (s) => s.status === 'CAMERA_READY',
    );
    
    // Then filter by search query if provided
    if (!searchQuery.trim()) {
      return cameraReadySubmissions;
    }
    const query = searchQuery.toLowerCase();
    return cameraReadySubmissions.filter(
      (s) =>
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Bài hoàn thiện (Camera-ready)</h1>
