...
describe('ReviewsService', () => {
  ...
  describe('checkConflictOfInterest', () => {
    it('returns true for conflict preference', async () => {
      ...
      await expect(service.checkConflictOfInterest(5, 'sub-3', 12)).resolves.toBe(true);
    });
    ...
    it('returns false when no conflict exists', async () => {
      ...
      await expect(service.checkConflictOfInterest(5, 33, 12)).resolves.toBe(false);
    });
  });
  ...
  describe('selfAssignSubmission', () => {
    it('rejects self assignment when conflict exists', async () => {
      ...
      await expect(service.selfAssignSubmission(1, 'sub-4', 2)).rejects.toBeInstanceOf(BadRequestException);
    });
  ...
  describe('getReviewsBySubmission', () => {
    it('returns enriched reviews with reviewer names', async () => {
      ...
      const result = await service.getReviewsBySubmission('submission-14', 1, 10, 'token-4');
    ...
    it('falls back to reviewer number when identity service fails', async () => {
      ...
      const result = await service.getReviewsBySubmission('submission-15', 1, 10, 'token-5');
    ...
    it('returns empty array when there are no assignments', async () => {
      ...
      await expect(service.getReviewsBySubmission('submission-16')).resolves.toEqual([]);
    });
  });
  ...
  describe('getDecisionSummaryBySubmission', () => {
    it('calculates statistics and returns decision', async () => {
      ...
      const result = await service.getDecisionSummaryBySubmission('submission-17');
