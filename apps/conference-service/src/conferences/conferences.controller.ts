...
export class ConferencesController {
  ...
  @ApiOperation({
    ...
    description:
      'Tạo một hội nghị mới. Người tạo sẽ tự động trở thành CHAIR của hội nghị.',
  })
  ...
  async findAll() {
    ...
    const conferencesWithDeadlines = conferences.map((conference) => ({
      ...conference,
      submissionDeadline:
        conference.cfpSetting?.submissionDeadline?.toISOString(),
      ...
      cameraReadyDeadline:
        conference.cfpSetting?.cameraReadyDeadline?.toISOString(),
    }));
    return {
  ...
  @ApiOperation({
    summary: 'Lấy danh sách chủ đề được phân công cho reviewer hiện tại',
  })
  @ApiResponse({
    status: 200,
  ...
  async getMyTrackAssignments(@CurrentUser() user: JwtPayload) {
    const assignments = await this.conferencesService.getMyTrackAssignments(
      user.sub,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Thiết lập hoặc cập nhật các mốc thời gian quan trọng cho CFP của hội nghị.',
  })
  @ApiParam({ name: 'id', description: 'ID của hội nghị' })
  @ApiResponse({ status: 201, description: 'Thiết lập CFP thành công' })
  @ApiResponse({
    status: 400,
    description: 'Dữ liệu không hợp lệ (thứ tự deadline không đúng)',
  })
  @ApiResponse({ status: 403, description: 'Không có quyền quản lý hội nghị' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy hội nghị' })
  async setCfp(
  ...
  @ApiOperation({
    ...
    description:
      'Cập nhật thông tin hội nghị. Tất cả các trường đều tùy chọn, chỉ cần gửi các trường muốn cập nhật.',
  })
  @ApiParam({ name: 'id', description: 'ID của hội nghị' })
  ...
  @ApiResponse({
    status: 400,
    description: 'Không thể xóa hội nghị vì đã có bài nộp',
  })
  @ApiResponse({ status: 403, description: 'Không có quyền quản lý hội nghị' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy hội nghị' })
  ...
  @ApiResponse({
    status: 404,
    description: 'Không tìm thấy hội nghị hoặc track',
  })
  async updateTrack(
  ...
  @ApiResponse({
    status: 400,
    description: 'Không thể xóa track vì đã có bài nộp',
  })
  @ApiResponse({ status: 403, description: 'Không có quyền quản lý hội nghị' })
  @ApiResponse({
    status: 404,
  ...
  @ApiOperation({
    summary: 'Kiểm tra reviewer đã chấp nhận track assignment chưa',
  })
  ...
  async checkReviewerTrackAssignment(
    ...
    const result = await this.conferencesService.checkReviewerTrackAssignment(
      reviewerId,
      trackId,
    );
  ...
  @ApiOperation({
    summary: 'Lấy danh sách thành viên ban chương trình của chủ đề',
  })
  @ApiParam({ name: 'trackId', description: 'ID của chủ đề' })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Dữ liệu không hợp lệ hoặc user đã là thành viên',
  })
  @ApiResponse({ status: 403, description: 'Không có quyền quản lý hội nghị' })
  @ApiResponse({ status: 404, description: 'Không tìm thấy chủ đề' })
  async addTrackMember(
    ...
    const authToken = authHeader?.startsWith('Bearer ')
      ? authHeader.substring(7)
      : undefined;
    const member = await this.conferencesService.addTrackMember(
      trackId,
      dto,
      {
        id: user.sub,
  ...
  @ApiResponse({
    status: 404,
    description: 'Không tìm thấy chủ đề hoặc thành viên',
  })
  async removeTrackMember(
    ...
    await this.conferencesService.removeTrackMember(
      trackId,
      userId,
      {
        id: user.sub,
  ...
  async acceptTrackAssignment(
    ...
    const member = await this.conferencesService.acceptTrackAssignment(
      trackId,
      user.sub,
    );
  ...
  async rejectTrackAssignment(
    ...
    const member = await this.conferencesService.rejectTrackAssignment(
      trackId,
      user.sub,
    );
